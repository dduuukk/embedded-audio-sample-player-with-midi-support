...
#endif
  /*
   * since the MPU is configured as write-through, see main.c file, there isn't
   * any need to maintain the cache as its content is always coherent with the
   * memory. If needed, check the file
   * "Middlewares/Third_Party/FatFs/src/drivers/sd_diskio_dma_template.c" to see
   * how the cache is maintained during the write operations.
   */
#if (ENABLE_SD_DMA_CACHE_MAINTENANCE == 1)
