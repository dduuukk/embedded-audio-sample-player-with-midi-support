 ...
 * It configures various settings such as synchronization, output drive, audio frequency,
 * mono/stereo mode, companding mode, tri-state, protocol, and data size.
 *
 ...
 * 
 * This function enables the clocks for SAI GPIOs (GPIOA, GPIOE, GPIOG) and the SAI interface (SAI1).
 */
void initClks() {
 ...
 *
 * This function initializes the DMA for the SAI peripheral. It configures the DMA
 * stream, sets the request to SAI1_B, sets the direction to memory to peripheral,
 * enables memory increment, sets the data alignment to word, sets the mode to circular,
 * sets the priority to high, and disables the FIFO mode. It is only called for 
 * SAI1_Block_B.
 *
 * @param hsai Pointer to the SAI handle structure.
 */
 ...
 * 
 * This function is called by the HAL library to perform any necessary initialization for the SAI peripheral.
 * It enables clocks, configures SAI pins, and initializes DMA for sai block B.
 * 
 * @param hsai Pointer to the SAI handle structure.
 */
 ...
 * 
 * This destructor performs necessary deinitialization for the SAI peripheral, including:
